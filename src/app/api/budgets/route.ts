...
export async function GET(request: NextRequest) {
  try {
    ...
    const actualsMap = await computeActualsForAccounts(user.id, accountIds, year);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!id) {
      ...
      return await cancelBudget(bodyId, user.id, request);
    }

    return await cancelBudget(id, user.id, request);
  } catch (error) {
...
async function computeActualsForAccounts(
  ...
  const accounts = await db.account.findMany({
    where: { id: { in: accountIds }, ...demoFilter },
