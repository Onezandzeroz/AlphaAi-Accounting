...
export async function GET(request: NextRequest) {
  try {
    ...
    if (type === 'income-statement') {
      return generateIncomeStatement(user.id, fromStr, toStr, toDate);
    } else {
      return generateBalanceSheet(user.id, toStr, toDate);
    }
  } catch (error) {
...
async function generateBalanceSheet(
  ...
  const allAccounts = await db.account.findMany({
    where: { userId, isActive: true, ...demoFilter },
  });
  ...
  const totalCurrentAssets = r(cash + bank + receivables + inventory + otherCurrentAssets);
  ...
  const totalShortTerm = r(payables + shortTermDebt + otherLiabilities);
  ...
  for (const [, acc] of accountBalances) {
    if (acc.group === AccountGroup.SHARE_CAPITAL) {
      shareCapital += getAccountBalance(acc.id);
    }
    ...
    if (acc.group === AccountGroup.RETAINED_EARNINGS) {
      ...
      if (acc.number === '3300') {
        currentYearResult = getAccountBalance(acc.id);
      } else {
        retainedEarnings += getAccountBalance(acc.id);
      }
    }
  }
  ...
  const REVENUE_GROUPS = [AccountGroup.SALES_REVENUE, AccountGroup.OTHER_REVENUE, AccountGroup.FINANCIAL_INCOME];
  const EXPENSE_GROUPS = [AccountGroup.COST_OF_GOODS, AccountGroup.PERSONNEL, AccountGroup.OTHER_OPERATING, AccountGroup.FINANCIAL_EXPENSE, AccountGroup.TAX];
