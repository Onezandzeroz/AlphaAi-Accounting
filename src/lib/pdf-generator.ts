...
export async function generateInvoicePDF(invoice: InvoiceWithDetails): Promise<Uint8Array> {
  ...
  if (company?.logo) {
    try {
      ...
      console.warn('[PDF] Failed to embed logo:', err);
    }
  }
  ...
  drawText(page, company?.companyName || 'AlphaAi Accounting', {
    x: companyNameX,
    y: companyNameY - 14,
    font: fontBold,
    size: 18,
  ...
  drawText(page, 'Enhedspris', { x: tableLeft + colDesc + colQty + 6, y: headerY, font: fontBold, size: headerSize, color: COLORS.primary, align: 'right', maxX: tableLeft + colDesc + colQty + colUnitPrice - 4 });
  drawText(page, 'Moms %', { x: tableLeft + colDesc + colQty + colUnitPrice + 4, y: headerY, font: fontBold, size: headerSize, color: COLORS.primary, align: 'center', maxX: tableLeft + colDesc + colQty + colUnitPrice + colVat - 4 });
  drawText(page, 'Linjetotal', { x: tableLeft + colDesc + colQty + colUnitPrice + colVat + 4, y: headerY, font: fontBold, size: headerSize, color: COLORS.primary, align: 'right', maxX: tableRight - 4 });
...
function drawFooter(page: PDFPage, fontBold: PDFFont, fontRegular: PDFFont, fontItalic: PDFFont, _pageNum: number) {
  ...
  // Branding text
  drawText(page, 'AlphaAi Accounting', {
    x: PAGE_WIDTH / 2,
    y: footerY + 8,
    font: fontBold,
    size: 9,
...
function parseLineItems(lineItemsStr: string): InvoiceLineItem[] {
  try {
    ...
    console.warn('[PDF] Failed to parse line items JSON');
  }
  return [];
}
