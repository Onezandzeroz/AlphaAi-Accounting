...
// VAT code validation for Danish rates
const VALID_DANISH_VAT_RATES = [0, 25];
...
export function validateSAFT(xmlContent: string): ValidationResult {
  ...
  if (!xmlContent.includes('<Header>')) {
    ...
    MANDATORY_HEADER_TAGS.forEach((tag) => {
      totalChecks++;
      const tagPattern = new RegExp(`<${tag.path.replace('/', '.*?<') ? tag.path.split('/').pop() : tag.path}>([^<]*)</${tag.path.split('/').pop()}>`, 'i');
...
export function validateTransactionData(transactions: Array<{
  ...
  transactions.forEach((t, index) => {
    ...
    } else if (!VALID_DANISH_VAT_RATES.includes(t.vatPercent)) {
      warnings.push(`${prefix} VAT rate ${t.vatPercent}% is not a standard Danish rate (0% or 25%)`);
    }
  });
...
export function logValidationResults(result: ValidationResult, context: string = 'SAF-T Validation'): void {
  ...
  console.log(`Summary: ${result.summary.passed}/${result.summary.totalChecks} checks passed`);
  console.log(`Errors: ${result.errors.length}, Warnings: ${result.warnings.length}`);
  
  if (result.errors.length > 0) {
    console.log('\nErrors:');
    result.errors.forEach((e, i) => {
      console.log(`  ${i + 1}. [${e.code}] ${e.path}: ${e.message}`);
  ...
  if (result.warnings.length > 0) {
    ...
    result.warnings.forEach((w, i) => {
      console.log(`  ${i + 1}. [${w.code}] ${w.path}: ${w.message}`);
