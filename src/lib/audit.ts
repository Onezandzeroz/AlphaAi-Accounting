...
export async function auditCreate(
  ...
  return auditLog({ action: 'CREATE', entityType, entityId, userId, changes, metadata: meta });
}
...
export async function auditUpdate(
  ...
  if (Object.keys(changes).length > 0) {
    return auditLog({ action: 'UPDATE', entityType, entityId, userId, changes, metadata: meta });
  }
}
...
export async function auditDeleteAttempt(
  ...
  return auditLog({ action: 'DELETE_ATTEMPT', entityType, entityId, userId, metadata: meta });
}
