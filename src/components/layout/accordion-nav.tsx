...
const NAV_SECTIONS: NavSectionDef[] = [
  {
    ...
    items: [
      ...
      { id: 'transactions', nameDa: 'Transaktioner', nameEn: 'Transactions', icon: Receipt },
      { id: 'invoices', nameDa: 'Fakturaer', nameEn: 'Invoices', icon: FileText },
      { id: 'contacts', nameDa: 'Kontakter', nameEn: 'Contacts', icon: Users },
      { id: 'recurring', nameDa: 'Gentagende', nameEn: 'Recurring', icon: RefreshCw },
    ],
  },
  {
    id: 'bookkeeping',
    nameDa: 'Bogføring & Regnskab',
  ...
  {
    id: 'reporting',
    nameDa: 'Rapportering & Analyse',
    nameEn: 'Reporting & Analysis',
    icon: FileBarChart,
    items: [
  ...
  {
    id: 'compliance',
    nameDa: 'Compliance & Afslutning',
    nameEn: 'Compliance & Closing',
    icon: ShieldCheck,
    items: [
  ...
  {
    id: 'maintenance',
    nameDa: 'Vedligehold & Sikkerhed',
    nameEn: 'Maintenance & Security',
    icon: Settings,
    items: [
      { id: 'settings', nameDa: 'Indstillinger', nameEn: 'Settings', icon: Settings },
            ...
            return (
              <AccordionItem
                ...
                className={cn(
                  ...
                  isExpanded
                    ? 'bg-[#eeeefa] dark:bg-[#28273e]'
                    : '',
                  isActive && !isExpanded
                    ? 'bg-[#eeeefa]/50 dark:bg-[#28273e]/50'
                    : ''
                )}
              >
                <AccordionTrigger
                  className={cn(
                    'px-3 py-2.5 rounded-lg hover:no-underline transition-all duration-200',
                    '[&[data-state=open]>svg]:rotate-180',
                    'hover:bg-[#eeeefa] dark:hover:bg-[#28273e]',
                    isExpanded
                      ? 'text-[#554fe9] dark:text-[#7a76f0]'
                      : 'text-slate-600 dark:text-gray-400'
                  )}
                >
                  <span className="flex items-center gap-2.5 text-[13px] font-semibold tracking-wide uppercase">
                    <SectionIcon
                      className={cn(
                    ...
                    />
                    <span>{isDa ? section.nameDa : section.nameEn}</span>
                  </span>
                </AccordionTrigger>
                      ...
                      return (
                        <button
                          ...
                          className={cn(
                            ...
                            isItemActive
                              ? 'bg-[#554fe9] text-white shadow-md'
                              : 'text-slate-600 dark:text-gray-300 hover:bg-[#eeeefa] dark:hover:bg-[#28273e] hover:text-[#2c2a27] dark:hover:text-white'
                          )}
                        >
                          <ItemIcon
                            className={cn(
                              ...
                              isItemActive
                                ? 'text-white'
                                : 'text-gray-400 dark:text-gray-500 group-hover:text-[#554fe9] dark:group-hover:text-[#7a76f0]'
                            )}
                          />
                          <span className="truncate">{itemName}</span>
                          {isItemActive && (
                            <span className="absolute right-2 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-white/80" />
                          )}
                        </button>
                      );
                    })}
